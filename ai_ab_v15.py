...
class AB:
    def __init__(self, pl, t=0.18, ponder=False):
        ...
        self.tt_move_used = 0
        self.tt_peak = 0
        # Ponder instrumentation. _in_ponder tags the thread context so
        # _tt_store and _s can attribute work correctly. ponder_keys is a
        # set of TT hash keys that were last written by the ponder thread;
        # cleared whenever the main search overwrites the same key.
        self._in_ponder = False
        self.ponder_keys = set()
        self.ponder_nodes = 0      # nodes searched inside _ponder_loop
        self.ponder_stores = 0     # TT writes during ponder
        self.ponder_hit_on_stored = 0       # main-search hits on ponder entry
        self.ponder_cutoff_on_stored = 0    # ponder entries that cut off
    ...
    def _s(self, state, depth, alpha, beta):
        ...
        if tt_entry:
            self.tt_hits += 1
            # Attribute to ponder only for main-search probes (not recursive
            # ponder self-hits — those are ponder reusing its own work, not
            # ponder paying off for the real search).
    ...
    def _tt_store(self, key, depth, value, flag, move):
        ...
        if prev is None or prev[0] <= depth:
            ...
            if self._in_ponder:
                ...
                # Main search overwrote this key — it's no longer a
                # ponder-origin entry.
